...
function Search() {
    ...
    $.ajax({
        ...
        success: function (data) {
            // Get search widget
            $.ajax({
                url: `${widgetApiUrl}&query=${q}`,
                type: 'GET',
                dataType: 'json',
                success: function (data) {
                    UpdateWidgetResults(data, q);
                }
            });
...
function QnaSearch() {
    ...
    $("#doc-details-div").removeClass("doc-details-div-fadeup");
    $("#doc-details-div").html("");
    ...
    // Get center of map to use to score the search results
    $.ajax({
        url: `${mrcApiUrl}&query=${q}&document=${doc}`,
        type: 'GET',
        dataType: 'json',
        success: function (data) {
            $('#loading-indicator').css("display", "none");
    ...
    //$.ajax({
    //    url: "http://2d74f226-a198-4f62-b92c-9e62d23e5134.eastasia.azurecontainer.io/score",
    //    type: 'POST',
    //    contentType: "application/json; charset=utf-8",
    //    headers: {
...
function UpdateQna(data, q) {
    console.log('qna result',data)
    
    //Results List
    UpdateQnaResults(data, q);
...
function DocumentEntitiesSearch(q) {
    //if (results && results.length > 0) {
    //    $('#loading-indicator').show();
    //}
    //else $('#progress-indicator').show();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            UpdateDocEntities(data, q);
        }
    });
}

function UpdateDocEntities(data, document) {
    console.log('doc entities result', data)

    //Results List
    UpdateDocEntitiesResults(data, document);
